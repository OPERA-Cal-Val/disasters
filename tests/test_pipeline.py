...
from disasters.pipeline import PipelineConfig, run_pipeline, run_download_only
...
def test_run_pipeline_multi_product_routing(mock_generate, mock_read_meta, mock_np, mock_auth, tmp_path):
    """Ensure run_pipeline correctly loops through a list of products and creates dedicated subdirectories."""
    mock_read_meta.return_value = pd.DataFrame({"Dataset": ["OPERA_L3_DSWX-HLS_V1", "OPERA_L2_RTC-S1_V1"]})
...
def test_run_download_only_accumulates_layers(mock_executor, mock_read_meta, mock_np, mock_auth, tmp_path):
...
def test_cli_multiple_flags():
    ...
    with patch("disasters.pipeline.run_search_only") as mock_search:
        ...
        result = runner.invoke(cli, [
            "search",
            "-b", "30.2 30.3 -97.8 -97.7",
            "-o", "/tmp/fake_out",
            "-p", "OPERA_L3_DSWX-HLS_V1",
            "-p", "OPERA_L2_RTC-S1_V1",
            "-s", "sentinel-1",
            "-s", "landsat"
        ])
