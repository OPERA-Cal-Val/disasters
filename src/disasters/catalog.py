...
def read_opera_metadata(output_dir: Path) -> pd.DataFrame:
    ...
    df_temp1 = pd.to_datetime(start_times, format=FORMAT_MICROSECONDS, errors="coerce")

    # Parse the RTC format (Non-RTC dates become NaT)
    df_temp2 = pd.to_datetime(start_times, format=FORMAT_SECONDS_ONLY, errors="coerce")
    ...
    if granule_col in df.columns:
        ...
        df["Scene_ID"] = df[granule_col].str.replace(
            r"(_\d{8}T\d{6}Z)(_\d{8}T\d{6}Z)", r"\1", regex=True
        )
        ...
        if dropped_count > 0:
            logger.info(
                f"Deduplicated {dropped_count} ghost granule(s) by keeping the newest processing dates."
            )

        # Clean up the temp column
        df = df.drop(columns=["Scene_ID"])
    else:
        logger.warning("Could not find Granule ID column. Skipping deduplication.")
...
def get_S1_orbit_direction(
    urls: list, username: str = None, password: str = None
) -> str:
    """
    ...
    try:
        ...
        direction = tags.get(
            "RTC_ORBIT_PASS_DIRECTION", tags.get("ORBIT_PASS_DIRECTION", "")
        ).lower()

        if "ascending" in direction:
            return "A"
        if "descending" in direction:
            return "D"
    except Exception as e:
